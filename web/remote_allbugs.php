...
<body>
	...
	<div style='width:100%;height:32px;'>
		...
		<div style='width:46%;float:right;margin-right:10px;height:32px;text-align:left;'>Hi <?=$username?> !<br/> Welcome to Bug Tracking Tool </div>
	</div>
...
if(isset($_REQUEST['project_name']) && $_REQUEST['project_name'] != ''){
	...
	// for page records
	$sql_query = "SELECT id, project, phase, module, topic, screen, reviewer, browser, bcat, severity, bdr, uploaddate, bugstatus FROM qcuploadinfo WHERE qc='".$username."' AND project='".$project_name."' ". $sql_where ." order by ".$sb." ".(($so=="a")?"asc":"desc");
	if(!$export){
	...
	if(mysql_num_rows($result) > 0){
		...
		if($export){
			...
			fputcsv($output, array('Bug Id','Project','Phase','Module','Topic','Screen','Reviewer','Browser','Bug Category','Severity','Description','Submission Date','Bug Status'));
		}
		...
		?>
			<tr>
				...
				<td><?=$row['bcat']?></td>
				<td><?=$row['severity']?></td>
				<td style="max-height: 100px;overflow: auto;display: block;font-size:10px"><?=$row['bdr']?></td>
				<td><?=$row['uploaddate']?></td>
				<td><?=$row['bugstatus']?></td>
				<td><a href="javascript:void(0)" onClick="loadIssue('<?=$row['id']?>')">Edit</a></td>
			</tr>
		<?php
			} else {
				fputcsv($output, $row);
...
?>
	...
	<div id="bugEditForm">
		...
		<form id="bug_edit_form" action="remote_bug_data.php" method="POST">
			...
			<table align="center" width="90%" cellpadding="6" style="margin-top:30px;">	
				<tr><td><b>Bug Status:</b><br/>
					...
					<span id="comments">
						...
						<textarea id="f_comments" name="comments" style="width:100%" rows="4"></textarea>
						<br />
...
$(document).ready(function() {
 $('#bug_edit_form').submit(function(event) {
	$.ajax({
            ...
            url         : 'remote_bug_data.php?'+$( "#bug_edit_form" ).serialize(), // the url where we want to POST
            encode          : true,
	    success: function(data){
				if(data == 'true') {
					alert('Bug Status has been changed');
					$('#bugEditForm').toggle('slide');
					location.reload(); 
				}
			}
