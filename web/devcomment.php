		...
		if($('#filter_name').val() != '') {
			...
			if(fvalue != '') {
				$.ajax({
					type: "GET",
					url: "getbugsdev.php",
					data: "show=" + numRecords + "&pagenum=" + pageNum+"&issuetype="+issuetype+"&q="+str+"&pro_id="+pro_id+"&r="+auditee+"&chd_id="+chd+"&filter_name="+$('#filter_name').val()+"&"+fname+"="+fvalue+"&bscat="+bscatval,
					cache: false,
					beforeSend: function() {
						$('.loader').show();
						$('.loaderParent').show();
...
function export123(){
	if($('#project').val() != '') {
		...
		var win = '';
		//var issuetype = 'any'; 
		//var issuetype = document.forms["tstest"]["bugStatus"].value;
		if($('#filter_name').val() != '' && $('#filter_name').val() != undefined && fvalue!='' && fvalue != undefined && fvalue != 'select') {
			window.open('exportbugs.php?mode=devbug&q='+$('#project').val()+'&pro_id='+pro_id+'&filter_name='+$('#filter_name').val()+'&'+fname+'='+fvalue+'&bscat='+bscatval+'&chd_id='+chd+'&r='+auditee);
		} else {
			window.open('exportbugs.php?mode=devbug&q='+$('#project').val()+'&pro_id='+pro_id+'&chd_id='+chd+'&r='+auditee);
		}
	} else {
...
function getFilter() {
	...
	if(pro != "Select") {
		...
		str=encodeURIComponent(str);
		xmlhttp.open("GET","getfilterinfo.php?q="+pro+"&pro_id="+pro_id+"&fil=dev",true);
		xmlhttp.send();
	} 
}
