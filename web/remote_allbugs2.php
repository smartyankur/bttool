...
<head>
	...
	<style>
		...
		#bugEditForm{
			...
			top:0px;
			right:0px;
			border-left:5px solid rgb(247,148,28);
			color:#FFF;
		}
		#bugEditForm td{
			color:#fff;
...
<body>
	...
	<table cellspacing="0" cellpadding="2" width="95%" border="1" style="border-collapse:collapse" align="center">
			<tr>
				...
				<th width="3%"><a href="remote_allbugs.php?project_name=<?=$project?>&page=<?=$page?>&sb=bugstatus&so=<?=($so=="a")?"d":"a"?>&filter_name=<?=$filter_name?>&<?=str_replace('filter_','',$filter_name)?>=<?=$filter_value?>">Bug Status</a></th>
...
if(isset($_REQUEST['project_name']) && $_REQUEST['project_name'] != ''){
	...
	$sql_query = "SELECT id, project, phase, module, topic, screen, browser, bcat, reviewer, bdr, severity, uploaddate, bugstatus FROM qcuploadinfo WHERE qc='".$username."' AND project='".$project_name."'". $sql_where;
	$result1 = mysql_query($sql_query);
	$total_records = mysql_num_rows($result1);

	// for page records
	$sql_query = "SELECT id, project, phase, module, topic, screen, browser, bcat, reviewer, bdr, severity, uploaddate, bugstatus FROM qcuploadinfo WHERE qc='".$username."' AND project='".$project_name."' ". $sql_where ." order by ".$sb." ".(($so=="a")?"asc":"desc");
	if(!$export){
	...
	if(mysql_num_rows($result) > 0){
		...
		?>
			<tr>
				...
				<td><?=$row['bugstatus']?></td>
				<td><a href="#" onClick="loadIssue('<?=$row['id']?>')">Edit</a></td>
			</tr>
		<?php
			} else {
				fputcsv($output, $row);
		...
		?>
			<tr>
				<td colspan="13" height="100px" align="center">No Comments found</td>
			</tr>
		<?php
	}
}
?>
	...
	<div id="bugEditForm">
		<div id="closebt">X</div>
		<form id="bug_edit_form" action="remote_bug_data.php" method="POST">
			<input type="hidden" id="f_bug_id" name="bug_id" value="">
			<input type="hidden" name="action" value="update">
			<table align="center" width="80%" cellpadding="6" style="margin-top:30px;">
				<tr><td><b>Project:</b> <br/><span id="f_project"></span></td></tr>
	
				<tr><td><b>Phase:</b> <br/><span id="f_phase"></span><br/></td></tr>
	
				<tr><td><b>Module:</b><br/><span id="f_module"></span><br /></td></tr>
	
				<tr><td><b>Topic:</b><br/><span id="f_topic"></span><br /></td></tr>
	
				<tr><td><b>Screen:</b><br/><span id="f_screen"></span><br /></td></tr>
	
				<tr><td><b>Browser:</b><br/><span id="f_browser"></span><br /></td></tr>
	
				<tr><td><b>Bug Category:</b><br/><span id="f_bugcat"></span><br /></td></tr>
	
				<tr><td><b>Reviewer:</b><br/><span id="f_reviewer"></span><br /></td></tr>
	
				<tr><td><b>Description:</b><br/><span id="f_description"></span><br /></td></tr>
	
				<tr><td><b>Severity:</b><br/><span id="f_severity"></span><br /></td></tr>
	
				<tr><td><b>Submission Date:</b><br/><span id="f_date"></span><br /></td></tr>
	
				<tr><td><b>Bug Status:</b><br/>
					<select id="f_bug_status" name="bug_status">
							<option value="hold">hold</option>
							<option value="open">open</option>
							<option value="closed">closed</option>
							<option value="ok as is">ok as is</option>
					...
					<span id="comments" style="display:none">
						<br /><b>Add Comments:</b><br />
						<textarea id="f_comments" name="comments" stlye="width:100%" rows="4"></textarea>
						<br />
	...
	<script>
		function loadIssue(bug_id){
			if(bug_id > 0){
				$('#bugEditForm').toggle('slide');
				$.get( "remote_bug_data.php?action=edit&bug_id="+bug_id, function( data ) {

					var fields = data.split("||");

					$("#f_bug_id").val(fields[0]);
					$("#f_project").html(fields[1]);
					$("#f_phase").html(fields[2]);
					$('#f_module').html(fields[3]);
					$('#f_topic').html(fields[4]);
					$('#f_screen').html(fields[5]);
					$('#f_browser').html(fields[6]);
					$('#f_bugcat').html(fields[7]);
					$('#f_reviewer').html(fields[8]);
					$('#f_description').html(fields[9]);
					$('#f_severity').html(fields[10]);
					$('#f_date').html(fields[11]);
					$('#f_bug_status').val(fields[12]);
				});
			}
		}
		$('#f_bug_status').change(function(){$('#comments').toggle('slide')});
		$('#closebt').click(function(){$('#bugEditForm').toggle('slide');});
$(function(){
    $("#bug_edit_form").submit(function(e) {       
      e.preventDefault();
    });
});
$(document).ready(function() {
 $('#bug_edit_form').submit(function(event) {
	...
	$.get({
            ...
            url         : 'remote_bug_data.php', // the url where we want to POST
            data        : $( "#bug_edit_form" ).serialize(), // our data object
            encode          : true,
