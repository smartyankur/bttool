...
if($_SERVER['REQUEST_METHOD'] == 'POST'){
 ...
 $rquery = "select * from lmsblob where project_id='$project_id' AND module='".stripslashes($module)."' AND submodule='".stripslashes($submodule)."' AND build='$build' AND type='$type' AND priority='$priority' AND severity='$severity' AND btype='$btype' AND reqid='$reqid' AND bdr='$bdr' AND dev='$dev'";
 ...
 $query="INSERT INTO lmsblob(reviewer, project_id, project, module, submodule, build, type, priority, grab, btype, reqid, bdr, entrydate, severity,dev) values('".$reviewer."', '".$project_id."', '".$project."','".$module."','".$submodule."','".$build."','".$type."','".$priority."','".$b."','".$btype."','".$reqid."','".$bdr."','".$currentdate."','".$severity."','".$dev."')";
...
function loadReqId(projectName, projectId){
  self.location='lmsbt.php?project='+ projectName +'&id='+ projectId;  
}
...
<TR>
	...
	<td>
    <?php
      $query = "select DISTINCT id, project from projecttask where status='accepted'";
      $retval = mysql_query( $query, $con );
      $count = mysql_num_rows($retval);
      ...
      if(mysql_num_rows($retval)){ 
        while($row = mysql_fetch_assoc($retval)){
          ...
          ?>
            <option<?php if($project==$row['project'])echo " selected";?> onClick="loadReqId('<?php echo $row['project']; ?>','<?php echo $row['id']; ?>')" ref="<?php echo $row['id']; ?>"><?php echo $row['project'];?></option> 
          <?php 
          }
        } 
      }else{
  ...
  <TD>
    <?php
      if( isset($_GET['id']) && !empty($_GET['id']) ){      
        $selectProject = "SELECT GROUP_CONCAT( DISTINCT `id` ORDER BY `id` DESC SEPARATOR ',' ) AS projectIds FROM projecttask WHERE id = '".$_GET['id']."' and status = 'accepted'";
