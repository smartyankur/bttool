    ...
    if( empty($errorMessage) ){
      ...
      //$cquery = "insert into ticket(user, ticket, timestamp, requestedby, officialemail, filepath, status, statusreason, rootcause, subrootcause, worklog, assignee, priority, lastmodifiedon, lastmodifiedby) values('$user', '$ticket', '$ADate', '$requested_by', '$official_email', '$fstr', 2, 2, 2, 2, 'dsfsfsdf', 'Sunil Kumar', priority, '$ADate', '$requested_by')";
        ...
        $IncidentNo = mysql_insert_id();
    		...
    		$testing = "wizdomsupport@gc-solutions.net";//"debasisp@gc-solutions.net";
    		...
    		$mailer->Username = "wizdomsupport@gc-solutions.net";//"debasisp@gc-solutions.net";
    	...
    	//	$mailer->AddCC($delhead, 'Anu');
   //    $mailer->AddCC($sfour, 'Sachin'); 
    		...
    		header ("Location: logticket.php?successMessage=".urlencode($successMessage));
        
    	}else{
        $errorMessage .= "Row couldn't be created";
      }
    }else{
    		header ("Location: logticket.php?errorMessage=".urlencode($errorMessage)."&tdesc=".$_POST['ticket']."&rqstby=".$_POST['requestedby']."&rqstremail=".$_POST['officialemail']);    
    }
}
	?>
<html>
...
<form name="tstest" id="tstest" enctype="multipart/form-data" method="post" action="" accept-charset="utf-8">
  <table class="table_text">
    <tr>
      ...
      <td><textarea name="ticket" id="ticket" rows="6" cols="30" maxlength="700" class="required" title="<font color='red'>Please enter ticket.</font>"><?php echo ( isset($_REQUEST['tdesc']) && !empty($_REQUEST['tdesc']) ) ? $_REQUEST['tdesc'] : ""; ?></textarea>      
      </td>
    </tr>
	<TR>
		<TD valign="top">Select a file:</TD>
    <TD><input type="file" name="userfile" id="userfile" size="35" title="<font color='red'>This field is required with a valid extension.</font>" accept="doc|docx|xls|xlsx|jpg|jpeg|png|bmp|gif|msg" /> <font color="grey">&nbsp;&nbsp;&nbsp;&nbsp;Accepted file format: .doc, .docx, .xls, .xlsx, .jpg, .jpeg, .png, .bmp, .gif, .msg. Size up to 1MB.</font>    
    </TD>
	</TR>
    <tr>
      <td valign="top">Requested by</td>
      <td><input type="text" name="requestedby" id="requestedby" maxlength="50" size="30" class="required" title="<font color='red'>Please enter your name.</font>" value="<?php echo ( isset($_REQUEST['rqstby']) && !empty($_REQUEST['rqstby']) ) ? $_REQUEST['rqstby'] : ""; ?>"></td>
    </tr>
    <tr>
      <td valign="top">Email ID</td>
      <td><input type="text" name="officialemail" id="officialemail" maxlength="50" size="30" class="required" email="true" title="<font color='red'>Enter valid email address.<br>&nbsp;&nbsp;&nbsp;&nbsp;For example mail_id@domain.com.</font>" value="<?php echo ( isset($_REQUEST['rqstremail']) && !empty($_REQUEST['rqstremail']) ) ? $_REQUEST['rqstremail'] : $email; ?>"></td>
    </tr>        
  </table>
  <br>
  <?php
