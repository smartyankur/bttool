...
function submitresponse(str)
{
  ...
  var status="stat"+str;
  //alert (status);
  var qtr=document.getElementById(status).value;
  //alert (qtr);
...
xmlhttp.open("GET","updatefs.php?q="+str+ "&r=" + ptr+ "&p=" + qtr,true);
xmlhttp.send();
...
xmlhttp.open("GET","updatefind.php?q="+str,true);
xmlhttp.send();
}
...
<?php	
    ...
    { 
     echo "<h3>"."Hi ".$row['username']." ! You Can Change The Finding Description and NC Type Here"."<h3>";
	 $username=$row['username'];
	 //$pwd=$row['pwd'];
    } 	
?>
<h3>Update NCs Individually</h3>
