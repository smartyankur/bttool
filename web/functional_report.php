<html>
<head>
<title>Functional Review Data</title>
<?php	
error_reporting(0);
session_start();
...
while($row = mysql_fetch_assoc($retval)){ 
  echo "<br>";
  echo "<br>"; 
  echo "<h4>"."Hi ".$row['username']." ! Welcome to Functional Report."."</h4>";
...
function displayRecords(numRecords = 10, pageNum = 1){
	...
	var tdate = trim(document.getElementById('TDate').value);
	if(project != "Select" ){
		$.ajax({
			type: "GET",
			url: "functional_report_data.php",
			data: "id="+pro_id+"&show="+numRecords,
			cache: false,
			beforeSend: function() {
				$('.loader').show();
				$('.loaderParent').show();
		...
		$.ajax({
			type: "GET",
			url: "functional_report_data.php",
			data: "fdate="+fdate+"&tdate="+tdate+"&show="+numRecords,
			cache: false,
			beforeSend: function() {
				$('.loader').show();
				$('.loaderParent').show();
...
function export123(){
	...
	var fdate = trim(document.getElementById('FDate').value);
	var tdate = trim(document.getElementById('TDate').value);
	window.open('export_functional_report.php?q='+$('#project').val()+'&pro_id='+pro_id+"&fdate="+fdate+"&tdate="+tdate);
...
<TD><input type="text" name="TDate" id="TDate" value="" maxlength="20" size="19" readonly="readonly">
  <a href="javascript:NewCal('TDate','ddmmyyyy')"><img src="cal.gif" width="16" height="16" border="0" alt="Pick a date"></a> 
</TD>
</TR>
</TABLE>
<br>
