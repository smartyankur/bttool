...
if($filename<>"")
{
   ...
   if(move_uploaded_file($_FILES['userfile']['tmp_name'],$upload_path . $str))
   {
    ...
    echo "</br>";
	$query="update  qcuploadinfo1 set phase='$f',module='$g',topic='$h',receivedate='$x',browser='$j',coursestatus='$k',bcat='$l',bdr='$m',asignee='$n',qc='$o',screen='$p',filepath='$str',filename='$filename',uploaddate='$mydate',severity='$q' where id='$d'";
	//echo $query;
...
//$date = date('m/d/Y h:i:s a', time());
	$mydate = date('Y-m-d h:i:s', time());     
    $query="update  qcuploadinfo1 set phase='$f',module='$g',topic='$h',receivedate='$x',browser='$j',coursestatus='$k',bcat='$l',bdr='$m',asignee='$n',qc='$o',screen='$p',uploaddate='$mydate',severity='$q' where id='$d'";
	//echo $query;
    if (mysql_query($query))
       {
        echo "Row Updated for ID =".$d;
